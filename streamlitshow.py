...
def save_result(defects, frame, manual=False):
    ...
    if defects:
        ...
        record = {"time": timestamp, "file": filename, "defect_type": "正常", "confidence": 1.0}
...
def yolo_detect(frame, conf_threshold):
    ...
    for box, cls, conf in zip(
        results.boxes.xyxy.cpu().numpy(), results.boxes.cls.cpu().numpy().astype(int), results.boxes.conf.cpu().numpy()
    ):
        ...
        if label == "logo":
            ...
            if crop.size > 0:
                cnn_result = cnn_classify(cv2.cvtColor(crop, cv2.COLOR_BGR2RGB))
                if cnn_result != "normal":
                    defects.append({"type": cnn_result, "bbox": [x1, y1, x2, y2], "confidence": 0.8})
...
def draw_results(frame, defects):
    ...
    for defect in defects:
        ...
        # 绘制边界框
        cv2.rectangle(frame, (x1, y1), (x2, y2), COLORS[label], style["thickness"])

        # 绘制标签
        (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, style["font_scale"], style["font_thickness"])
        # 检查文字位置是否在视频画面内
        if y1 - th - 5 < 0:
            y1 = th + 5
        ...
        cv2.rectangle(frame, (x1, y1 - th - 5), (x1 + tw, y1), COLORS[label], -1)
        cv2.putText(
            frame,
            label,
            (x1, y1 - 5),
...
if detection_mode == "实时摄像头":
    ...
    if st.session_state.capture:
        while st.session_state.capture:
            ...
            # 显示原视频和检测后视频
            original_camera_feed.image(
                cv2.cvtColor(original_frame, cv2.COLOR_BGR2RGB), channels="RGB", use_container_width=True
            )
            detected_camera_feed.image(
                cv2.cvtColor(detected_frame, cv2.COLOR_BGR2RGB), channels="RGB", use_container_width=True
            )
            ...
            # 手动保存处理
            if manual_save and (st.session_state.get("space_pressed") or save_btn):
    ...
    if uploaded_file is not None:
        ...
        # 执行检测
        defects = yolo_detect(cv2.cvtColor(frame, cv2.COLOR_RGB2BGR), confidence_threshold)
        detected_frame = draw_results(cv2.cvtColor(frame, cv2.COLOR_RGB2BGR), defects)

        # 显示原视频和检测后视频
        col1.image(cv2.cvtColor(original_frame, cv2.COLOR_BGR2RGB), caption="原图片", use_container_width=True)
        col2.image(cv2.cvtColor(detected_frame, cv2.COLOR_BGR2RGB), caption="检测后图片", use_container_width=True)
    ...
    if uploaded_video is not None:
        ...
        # 添加进度条
        st.session_state.current_frame = st.slider("视频进度", 0, total_frames - 1, st.session_state.current_frame)
